   ...
   if (q->alpha_encode) {
    ...
    if (q->scc) {
        if (QSV_RUNTIME_VERSION_ATLEAST(q->ver, 2, 10)) {
            ...
            q->extsccparam.Palette = MFX_CODINGOPTION_ON;
            q->extsccparam.IntraBlockCopy = MFX_CODINGOPTION_ON;
...
static int qsv_retrieve_enc_params(AVCodecContext *avctx, QSVEncContext *q)
{
    ...
    mfxExtBuffer *ext_buffers[6 + QSV_HAVE_HE + QSV_HAVE_AC + QSV_HAVE_EXT_MSE];
